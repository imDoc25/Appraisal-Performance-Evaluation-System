...
Class Action {
	...
	function save_subject(){
		...
		$chk = $this->db->query("SELECT * FROM subject_list where (".str_replace(",",'and',$data).") and id != '{$id}' ")->num_rows;
		if($chk > 0){
			return 2;
		}
		if(empty($id)){
	...
	function save_category(){
		...
		$chk = $this->db->query("SELECT * FROM category_list where (".str_replace(",",'and',$data).") and id != '{$id}' ")->num_rows;
		if($chk > 0){
			return 2;
		}
		if(isset($user_ids)){
	...
	function save_promotion(){
		...
		$chk = $this->db->query("SELECT * FROM promotion_list where (".str_replace(",",'and',$data).") and id != '{$id}' ")->num_rows;
		if($chk > 0){
			return 2;
		}
		$hasDefault = $this->db->query("SELECT * FROM promotion_list where is_default = 1")->num_rows;
		if($hasDefault == 0){
			$data .= " , is_default = 1 ";
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO promotion_list set $data");
		}else{
			$save = $this->db->query("UPDATE promotion_list set $data where id = $id");
		}
		if($save){
			return 1;
		}
	...
	function delete_promotion(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM promotion_list where id = $id");
		if($delete){
			return 1;
		}
	}
	function save_attendance(){
		...
		$chk = $this->db->query("SELECT * FROM attendance_list where (".str_replace(",",'and',$data).") and id != '{$id}' ")->num_rows;
		if($chk > 0){
			return 2;
		}
		$hasDefault = $this->db->query("SELECT * FROM attendance_list where is_default = 1")->num_rows;
		if($hasDefault == 0){
			$data .= " , is_default = 1 ";
		}
		if(empty($id)){
	...
	function save_award(){
		...
		$chk = $this->db->query("SELECT * FROM awardee_list where (".str_replace(",",'and',$data).") and id != '{$id}' ")->num_rows;
		if($chk > 0){
			return 2;
		}
		$hasDefault = $this->db->query("SELECT * FROM awardee_list where is_default = 1")->num_rows;
		if($hasDefault == 0){
			$data .= " , is_default = 1 ";
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO awardee_list set $data");
		}else{
			$save = $this->db->query("UPDATE awardee_list set $data where id = $id");
		}
		if($save){
			return 1;
		}
	...
	function delete_award(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM awardee_list where id = $id");
		if($delete){
			return 1;
		}
	}
	...
	function save_erestriction(){
		extract($_POST);
		$filtered = implode(",",array_filter($rid));
		if(!empty($filtered))
			$this->db->query("DELETE FROM erestriction_list where id not in ($filtered) and academic_id = $academic_id");
		else
			$this->db->query("DELETE FROM erestriction_list where  academic_id = $academic_id");
		foreach($rid as $k => $v){
			$data = " academic_id = $academic_id ";
			$data .= ", efaculty_id = {$efaculty_id[$k]} ";
			$data .= ", categ_id = {$categ_id[$k]} ";
			$data .= ", esubject_id = {$esubject_id[$k]} ";
			if(empty($v)){
				$save[] = $this->db->query("INSERT INTO erestriction_list set $data ");
			}else{
				$save[] = $this->db->query("UPDATE erestriction_list set $data where id = $v ");
			}
		}
			return 1;
	}
	...
	function save_evaluations(){
		...
		$data .= ", emprestriction_id = $emprestriction_id ";
		$data .= ", faculty_id = $faculty_id ";
		$save = $this->db->query("INSERT INTO empevaluation_list set $data");
		if($save){
			$eid = $this->db->insert_id;
			foreach($qid as $k => $v){
				$data = " empevaluation_id = $eid ";
				$data .= ", question_id = $v ";
				$data .= ", rate = {$rate[$v]} ";
				$ins[] = $this->db->query("INSERT INTO empevaluation_answers set $data ");
			}
			if(isset($ins))
				return 1;
		}
